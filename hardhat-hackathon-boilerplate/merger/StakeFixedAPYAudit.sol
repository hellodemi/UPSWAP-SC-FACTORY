...
contract StakeFixedAPY is AccessControl, ReentrancyGuard, Ownable {
    ...
    event WithdrawTime(address indexed wallet, uint256 amount, uint256 date);
    ...
    event SetYearToSeconds( uint256 _seconds);
    ...
    constructor(address _stakingToken, address _rewardToken) {
        ...
        // set some defaults
        lockTimePeriod = 604800; // 7 days in seconds
        stakeRewardAPY = 15; // 15%
    ...
    function setSecondsInYear(uint256 _seconds) external onlyOwner {
        ...
        emit SetYearToSeconds(_seconds);
    }
    ...
    function _stakeRewards() internal returns (uint256) {
        ...
        emit Deposit(msg.sender, earnedRewardTokens);
    ...
    function _withdraw() internal returns (uint256) {
        ...
        emit WithdrawTime(msg.sender, amount, block.timestamp); // = user.stakeTime
        return amount;
    }
