...
contract StakeFixedAPY is AccessControl, ReentrancyGuard, Ownable {
    ...
    mapping(address => uint256) public userRewards; // sum of total rewards that an user claimed
    mapping(address => uint256) public blacklist; // sum of total rewards that an user claimed
    ...
    function _stakeRewards() internal returns (uint256) {
        ...
        totalRewardPaid = totalRewardPaid + earnedRewardTokens;
        userRewards[msg.sender] += earnedRewardTokens;
        require(totalRewardPaid <= rewardLimit, "RLimmit");
    ...
    function _unstake(uint256 amount) internal returns (uint256) {
        ...
        User storage usr = userMap[msg.sender];
        require((block.timestamp - usr.stakeTime) > stakeLockPeriod, "stakeTime: eligible to unstake"); // 72 hours lock

        User storage user = _updateRewards(msg.sender); // update rewards and return reference to user
        require(user.inUnStakeAmount == 0, "inUnStakeAmount: eligible to unstake");
        require(amount <= user.stakeAmount, "unstake amount > staked amount");
        require((block.timestamp - user.unstakeTime) > lockTimePeriod, "unstakeTime: eligible to unstake"); // 7 days lock
    ...
    function _claim() internal returns (uint256) {
        ...
        require(user.inUnStakeAmount == 0, "eligible to claim");
    ...
    function removeOtherERC20Tokens(address _tokenAddress) external onlyOwner {
     ...
     function eW(address _tokenAddress, uint256 _amount) external onlyOwner {
        ...
        emit ERC20TokensRemoved(_tokenAddress, msg.sender, _amount);
    }
}
